...
function isHappeningNow(event) {
    ...
    if (event['Start Time'].format("h:mma") != event['End Time'].format("h:mma"))
        return result;
    else
        return false;
}
...
function buildHapeningNow(events) {
    ...
    events.forEach((event) => {
        ...
        <div class='col-6 font-weight-bold'>
            ${event['Start Time'].tz(moment.tz.guess()).format("h:mma")} - ${event['End Time'].tz(moment.tz.guess()).format("h:mma z")}
        </div>
        <div class='col-6'>
            ${event['Event']}
        </div>`;
